...
class ProductSimplex:
    ...
    def to_ordered_simplex(self):
        simplex = self.sigma if len(self.sigma) == self.dim + 1 else self.sigma + [0] * (
                    self.dim + 1 - len(self.sigma))  # append zeros
...
class BlowupComplex():
    ...
    def compute_persistence(self, verbose=False, show_diag=True, compute_basis=True):
        ...
        """
        print('number of barcode',
              np.sum([len(x) for t, x in self.dgms.items()]))  # if show_diagonal is true, equals the number of pairs
